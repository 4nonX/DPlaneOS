...
// ParseSGSesOutput parses the text output of `sg_ses --page=es <device>`.
// The parser is lenient: it captures what it understands and skips the rest.
func ParseSGSesOutput(output string) []SESElement {
	var elements []SESElement
	var curType, curDesc, curStatus, curDetails string
	...
	for _, raw := range strings.Split(output, "\n") {
		...
		// "Element type: Foo, subenclosure id: ..."
		if strings.HasPrefix(line, "Element type:") {
			parts := strings.SplitN(line, ",", 2)
			curType = strings.TrimSpace(strings.TrimPrefix(parts[0], "Element type:"))
			continue
		}
		// "Element N descriptor: Name" or "Overall descriptor: Name, additional..."
		if strings.Contains(line, "descriptor:") {
			parts := strings.SplitN(line, "descriptor:", 2)
			if len(parts) == 2 {
				flush()
				desc := strings.TrimSpace(parts[1])
				// Strip trailing metadata like ", additional length=0, EIP=0"
				if idx := strings.Index(desc, ", additional"); idx > 0 {
					desc = desc[:idx]
				}
				curDesc = desc
			}
		...
		// First content line after descriptor becomes the status.
		if curStatus == "" {
			curStatus = line
			continue
		}
