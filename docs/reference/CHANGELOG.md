...
- **SES topology and status API**: `GET /api/enclosure` returns all enclosures and their slots. `GET /api/enclosure/{id}/ses-status` runs `sg_ses --page=es` against the resolved `/dev/sgN` device and returns parsed element status. `/dev/sgN` path is validated against a strict regex before exec. Requires `system:read` permission.
