...
1. **Blocking HTTP operations** — 8 long-running API endpoints returned responses only after the operation completed (up to 10 minutes). The browser held open HTTP connections for the entire duration, causing timeout errors on large image pulls, ZFS send/receive operations, and compose deployments. These are now fully async with in-UI job polling.
...
**Affected file:** `app/assets/js/ui-components.js`
...
These endpoints were made async in v3.3.2 on the backend — they now return `{"job_id":"<uuid>"}` immediately. v3.3.3 completes the frontend side.
